...
def update_product_list():
    ...
    # Load products from CSV file using read_products
    product_list = read_products(CSV_FILE)

    # Add products to the listbox with shortened URLs
    for product in product_list:
        display_url = shorten_url(product['url'])
        product_listbox.insert(tk.END, f"{display_url} (Target: ${product['target_price']})")
...
# Function to append messages to the log_text with color-coding and spacing
def log_message(message_type, message):
    """
    Logs messages to the scrolledtext widget with color-coding and spacing.
...
def send_email(subject, body):
    ...
    try:
        ...
        server.quit()
        # Log the email sent as SUCCESS
        gui_queue.put(("SUCCESS", f"Email sent: {subject}\n"))
    except Exception as e:
        # Log the email failure as ERROR
...
def get_product_name(url):
    ...
    try:
        ...
        if title:
            ...
            gui_queue.put(("INFO", f"Fetched product name: '{product_name}'"))
            return product_name
        else:
...
# Define tags for color-coding and spacing
...
log_text.tag_config("INFO", foreground="blue", spacing1=2, spacing2=2, spacing3=10)
log_text.tag_config("SUCCESS", foreground="green", spacing1=2, spacing2=2, spacing3=10)
log_text.tag_config("ERROR", foreground="red", spacing1=2, spacing2=2, spacing3=10)
...
# Set up protocol for graceful closing
